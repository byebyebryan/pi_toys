...
if __name__ == '__main__':
    import dispy, random, argparse, resource, cluster_config
    ...
    cluster = dispy.JobCluster(compute, nodes=cluster_config.nodes, ip_addr=cluster_config.manager_ip)
    print(('submitting %i jobs of %i points each' % (no_of_jobs, no_of_points)))
    jobs = []
    ...
    for job in jobs:
        ...
        print(('executed job %s using %i with result %i' % (job.id, ran_seed, inside)))
